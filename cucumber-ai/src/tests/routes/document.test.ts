import type { Document } from '@/lib/chatbot/db/schema';
import { generateUUID } from '@/lib/chatbot/utils';
import { expect, test } from '../fixtures';
import { getMessageByErrorCode } from '@/lib/chatbot/errors';

const documentsCreatedByAda: Array<Document> = [];

test.describe
  .serial('/api/chatbot/document', () => {
    test('Ada cannot retrieve a document without specifying an id', async ({
      adaContext,
    }) => {
      const response = await adaContext.request.get('/api/chatbot/document');
    ...
    test('Ada cannot retrieve a document that does not exist', async ({
      ...
      const response = await adaContext.request.get(
        `/api/chatbot/document?id=${documentId}`,
      );
    ...
    test('Ada can retrieve a created document', async ({ adaContext }) => {
      ...
      const response = await adaContext.request.get(
        `/api/chatbot/document?id=${document.id}`,
      );
      expect(response.status()).toBe(200);
    ...
    test('Ada can save a new version of the document', async ({
      ...
      const response = await adaContext.request.post(
        `/api/chatbot/document?id=${firstDocument.id}`,
        {
          data: draftDocument,
        },
      );
    ...
    test('Ada cannot delete a document without specifying an id', async ({
      ...
      const response = await adaContext.request.delete(`/api/chatbot/document`);
      expect(response.status()).toBe(400);
    ...
    test('Ada can delete a document by specifying id and timestamp', async ({
      ...
      const response = await adaContext.request.delete(
        `/api/chatbot/document?id=${firstDocument.id}&timestamp=${firstDocument.createdAt}`,
      );
      expect(response.status()).toBe(200);
